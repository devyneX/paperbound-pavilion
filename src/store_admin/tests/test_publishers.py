...
class TestPublisherViews(TestCase):
    ...
    def setUp(self):
        ...
        self.publisher_instance = Publisher.objects.create(
            name='Existing Publisher', address='456 Existing Street'
        )
    ...
    def test_publishers_list_view(self):
        ...
        self.assertTemplateUsed(
            response, 'store_admin/publisher/publishers.html'
        )

    def test_publishers_create_view(self):
        response = self.client.post(
            reverse('admin-publisher-create'), data=self.publisher_data
        )
    ...
    def test_publisher_detail_view(self):
        response = self.client.get(
            reverse(
                ...
                kwargs={'pk': self.publisher_instance.pk}
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, 'store_admin/publisher/publisher-detail.html'
        )

    def test_publisher_delete_view(self):
        response = self.client.post(
            reverse(
                'admin-publisher-delete',
                kwargs={'pk': self.publisher_instance.pk}
            )
        )
        self.assertEqual(
    ...
    def test_publishers_update_view(self):
        ...
        # updated_publisher = Publisher.objects.get(
        # pk=self.publisher_instance.pk)
