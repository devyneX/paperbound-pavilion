...
class TestUserViews(TestCase):
    def setUp(self):
        ...
        self.user_instance = User.objects.create_user(username='existinguser', email='existinguser@example.com', password='existingpassword')
    ...
    def test_users_create_view(self):
        response = self.client.post(reverse('admin-user-create'), data=self.user_data)
        self.assertEqual(response.status_code, 200) 
    ...
    def test_users_update_view(self):
        response = self.client.post(reverse('admin-user-update', kwargs={'pk': self.user_instance.pk}), data=self.user_update_data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_detail_view(self):
        response = self.client.get(reverse('admin-user-detail', kwargs={'pk': self.user_instance.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'store_admin/user/user-detail.html')

    def test_user_delete_view(self):
        response = self.client.post(reverse('admin-user-delete', kwargs={'pk': self.user_instance.pk}))
