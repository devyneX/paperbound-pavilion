...
class TestAuthorViews(TestCase):
    ...
    def test_authors_create_view(self):
        response = self.client.post(reverse('admin-author-create'), data=self.author_data)
    ...
    def test_author_detail_view(self):
        response = self.client.get(reverse('admin-author-detail', kwargs={'pk': self.author_instance.pk}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'store_admin/author/author-detail.html')

    def test_author_delete_view(self):
        response = self.client.post(reverse('admin-author-delete', kwargs={'pk': self.author_instance.pk}))
    ...
    def test_authors_update_view(self):
        ...
        response = self.client.post(reverse('admin-author-update', kwargs={'pk': self.author_instance.pk}), data=updated_data)
        self.assertEqual(response.status_code, 302)
